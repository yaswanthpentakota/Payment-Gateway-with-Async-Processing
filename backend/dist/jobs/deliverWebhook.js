...
const deliverWebhook = async (job) => {
    ...
    try {
        // 1. Fetch Merchant
        const merchantRes = await (0, db_1.query)('SELECT webhook_url, webhook_secret FROM merchants WHERE id = $1', [merchantId]);
        ...
        if (!logId) {
            const logRes = await (0, db_1.query)('INSERT INTO webhook_logs (merchant_id, event, payload, status, attempts) VALUES ($1, $2, $3, $4, $5) RETURNING id', [merchantId, event, JSON.stringify(payload), 'pending', 0]);
            logId = logRes.rows[0].id;
        }
        // 3. Prepare Request
        const payloadString = JSON.stringify(payload);
        const signature = (0, utils_1.generateSignature)(payloadString, merchant.webhook_secret);
        // 4. Send Request
        let responseCode = 0;
        let responseBody = '';
        let success = false;
        try {
            const res = await axios_1.default.post(merchant.webhook_url, payload, {
                headers: {
        ...
        await (0, db_1.query)('UPDATE webhook_logs SET status = $1, response_code = $2, response_body = $3, attempts = attempts + 1, last_attempt_at = NOW() WHERE id = $4', [success ? 'success' : (attempt >= 5 ? 'failed' : 'pending'), responseCode, responseBody, logId]);
        // 6. Handle Retry
        if (!success) {
            if (attempt < 5) {
                // Calculate delay
                const delays = utils_1.WEBHOOK_RETRY_TEST
                ...
                await (0, db_1.query)('UPDATE webhook_logs SET next_retry_at = $1 WHERE id = $2', [nextRetryAt, logId]);
